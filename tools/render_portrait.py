...
def load_source_image() -> Image.Image:
    ...
    for y in range(size):
        tone = int(12 + (y / (size - 1)) * 16)
        draw.line((0, y, size, y), fill=tone)

    draw.ellipse((64, 26, 256, 240), fill=44)
    draw.ellipse((88, 50, 232, 232), fill=168)
    draw.ellipse((76, 30, 244, 178), fill=58)
    draw.ellipse((104, 94, 128, 120), fill=24)
    draw.ellipse((192, 94, 216, 120), fill=24)
    draw.rectangle((98, 142, 126, 148), fill=96)
    draw.rectangle((192, 142, 220, 148), fill=96)
    draw.line((140, 156, 180, 174), fill=80, width=3)
    draw.arc((142, 168, 188, 208), 18, 162, fill=58, width=3)
    draw.rectangle((122, 194, 198, 252), fill=112)
    draw.pieslice((44, 148, 276, 318), 200, -20, fill=78)
    draw.ellipse((52, 140, 268, 302), fill=62)
...
def remap_tone(value: int) -> int:
    ...
    normalized = max(0.0, min(1.0, (normalized - 0.08) * 1.18))
    normalized = normalized ** 0.84
...
def build_svg(lines: list[str]) -> str:
    width = 42 * 8 + 48
    height = MARGIN_Y * 2 + len(lines) * LINE_HEIGHT + 38
    ...
    for index, line in enumerate(lines):
        ...
        body_lines.append(
            ...
            f'fill="{FG}" xml:space="preserve">{escaped_line}'
  ...
  <style>
    <![CDATA[
      .frame-glow {{ animation: pulse 3.8s ease-in-out infinite; }}
  ...
  <defs>
    ...
    <radialGradient id="glow" cx="50%" cy="35%" r="70%">
      <stop offset="0%" stop-color="#2ee59d" stop-opacity="0.2" />
      <stop offset="100%" stop-color="#2ee59d" stop-opacity="0" />
    </radialGradient>
    <clipPath id="reveal">
      <rect x="18" y="18" width="{width - 36}" height="0">
        <animate attributeName="height" from="0" to="{reveal_height}" dur="2.8s" begin="0s" fill="freeze" />
      </rect>
    </clipPath>
    <pattern id="scanlines" width="100%" height="8" patternUnits="userSpaceOnUse">
      <rect width="100%" height="1" fill="#ffffff" opacity="0.04" />
    </pattern>
  </defs>
  ...
  <rect x="18" y="36" width="{width - 36}" height="{height - 54}" rx="8" fill="url(#scanlines)" opacity="0.46" class="scan" />
  ...
  <rect x="{width - 28}" y="38" width="2" height="16" fill="#8fffb7" class="cursor" />
</svg>
'''
...
def main() -> None:
    ...
    lines = build_ascii_lines(source)
